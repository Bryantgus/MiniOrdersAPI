  ...
  <section class="flex flex-col items-center justify-center bg-stone-500 rounded-xl p-6 gap-3 shadow shadow-stone-400">
    ...
    <form @submit.prevent="postOrder" class="flex flex-col gap-2">
      <div class="flex flex-col gap-1">
        ...
        <input v-model="nombre" class="focus:outline focus:outline-stone-300 p-1 bg-stone-400 rounded-xl" placeholder="Ingresar nombre" type="text">
      </div>
      <div class="flex flex-col gap-1">
        <span class="text-[18px] font-semibold">Total</span>
        <input v-model="total" class="focus:outline focus:outline-stone-300 p-1 bg-stone-400 rounded-xl" placeholder="Ingresar el Total" type="text">
      </div>
      <button type="submit" class="cursor-pointer bg-stone-700 mt-10 mb-5 text-white p-2 rounded hover:bg-stone-600">Agregar Orden</button>
    </form>
  </section>
</template>
